...
// @resource     jurishtml https://raw.githubusercontent.com/afhill/userscript-ln/master/juris.html?v=4
...
(function() {
    ...
    function runMain() {
        ...
        $(".highlanderpod, .getadoc, .advancesearch, .searchbox .options").remove();
...
//        $('.searchsection > div').prepend('<div class="juris prefilter"><label for="juris">within</label><div class="selecty"><a class="selecty-selected" data-placeholder="All Jurisdictions and Courts"><i>All Jurisdictions and Courts</i></a></div></div>');
        ...
        $('.searchsection > div').prepend('<div class="hlct prefilter"><label for="hlct">starting in</label><select id="hlct"><option value="urn:hlct:1:11">CA Cases</option><option value="urn:hlct:1:12">CA Legislation</option><option value="urn:hlct:1:13">CA Quantums</option><option value="urn:hlct:1:14">CA Drafting Materials</option><option value="urn:hlct:1:15">CA Secondary Materials</option><option value="urn:hlct:1:16">CA Dictionaries</option></select></div>');
        ...
        $("#searchTerms")
            .val("")
            .attr("placeholder","Enter terms, sources, a citation, or cit: to note up with QuickCITE®")
            .keyup(function(e) {
        ...
        $('.pod-wrapper.browse')
            .wrap( "<div class='exploresection'><div></div></div>" )
            .prepend('<h2 class="sectionheader">Explore Content</h2>');
    ...
    function sourceCheck() {
        if(!$(".appliedfilters > button").length) {
            ...
            $(".juris .selecty-selected").html("All Courts and Jurisdictions");
            $('#clearform')[0].click();
        }
    }
